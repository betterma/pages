...
const FAVORITES_PATH = process.env.OKX_FAVORITES_PATH || 'okx/favorites.json';
const CANDLES_PATH = process.env.OKX_CANDLES_PATH || 'okx/candles-cache.json';
...
async function saveJson(path, data, sha, message) {
  ...
  if (!response.ok) {
    throw new Error(
      `GitHub PUT ${path} failed: ${response.status} ${response.text().slice(0, 240)}`,
    );
  }
